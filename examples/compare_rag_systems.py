...
class RAGComparison:
    ...
    def compare_on_question(self, question: str) -> Dict[str, Any]:
        ...
        print(f"Done! ({simple_result['total_chunks']} chunks, {simple_result['retrieval_time']:.3f}s)")

        # Run MeVe RAG
        print("Running MeVe RAG...", end=" ")
        meve_result = self.meve_rag.retrieve(question)
        print(f"Done! ({meve_result['total_chunks']} chunks, {meve_result['total_tokens']} tokens, {meve_result['retrieval_time']:.3f}s)")
        print()

        return {
            "question": question,
            "simple_rag": simple_result,
            "meve_rag": meve_result
        }
    ...
    def create_comparison_report(self, results: List[Dict[str, Any]], output_file: str) -> None:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if simple_times:
                ...
                f.write(f"  Average chunks retrieved: {sum(simple_chunks)/len(simple_chunks):.1f}\n")
                ...
                f.write(f"  MeVe is {sum(meve_times)/sum(simple_times):.2f}x slower (due to 5-phase processing)\n")
            ...
            for i, result in enumerate(results, 1):
                ...
                f.write("SIMPLE RAG:\n")
                f.write("-" * 80 + "\n")
                f.write(f"Chunks: {result['simple_rag']['total_chunks']}, Time: {result['simple_rag']['retrieval_time']:.3f}s\n\n")
                f.write("Sources:\n")
                ...
                f.write("MEVE RAG:\n")
                f.write("-" * 80 + "\n")
                f.write(f"Chunks: {result['meve_rag']['total_chunks']}, Tokens: {result['meve_rag']['total_tokens']}, Time: {result['meve_rag']['retrieval_time']:.3f}s\n\n")
                f.write("Sources:\n")
                for j, chunk in enumerate(result['meve_rag']['chunks'], 1):
                    f.write(f"  {j}. {chunk['title']} (relevance: {chunk['relevance_score']:.3f}, {chunk['token_count']} tokens)\n")
                f.write("\n")

                # Comparison
                f.write("ANALYSIS:\n")
                f.write("-" * 80 + "\n")
                time_diff = result['meve_rag']['retrieval_time'] - result['simple_rag']['retrieval_time']
                f.write(f"Time: MeVe {time_diff:+.3f}s ({result['meve_rag']['retrieval_time']/result['simple_rag']['retrieval_time']:.2f}x)\n")
                f.write(f"Chunks: Simple={result['simple_rag']['total_chunks']}, MeVe={result['meve_rag']['total_chunks']}\n")
