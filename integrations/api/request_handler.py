...
class RequestHandler:
    ...
    def execute_query(
        ...
        try:
            ...
            # Run the engine
            context_str, chunks_list = self.engine.run(query)
            ...
            # Format chunks from the returned list
            chunks = self._format_chunks(chunks_list)

            # Calculate metrics
            metrics = self._extract_metrics(chunks_list)
    ...
    def _extract_metrics(self, chunks_list=None) -> Dict[str, Any]:
        ...
        # Use provided chunks_list or fall back to engine's last_retrieved_chunks
        chunks = chunks_list or self.engine.last_retrieved_chunks
        if not chunks:
            return metrics
        ...
        metrics["phase_5_final_tokens"] = sum(
            chunk.token_count or 0 for chunk in chunks
        )
