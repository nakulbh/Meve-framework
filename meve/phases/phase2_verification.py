    ...
    try:
        ...
        for chunk, score in zip(initial_candidates, scores):
            ...
            if isinstance(score, torch.Tensor):
                # Handle torch tensor - extract scalar value
                score = score.item() if score.numel() == 1 else score.flatten()[0].item()
                ...
                try:
                    ...
                    # If item() fails (multi-element array), flatten and take first
                    score = float(np.asarray(score).flatten()[0])
            else:
                score = float(score)
