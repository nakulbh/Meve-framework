...
class RetrievalMetrics:
    ...
    def log_summary(self, logger) -> None:
        ...
        if self.token_budget:
            logger.info(f"   Budget: {self.token_budget} (Efficiency: {self.budget_efficiency:.1f}%)")
...
class ComparisonMetrics:
    ...
    def log_comparison(self, logger) -> None:
        ...
        # Timing
        logger.info(f"{'Total Time':<30} {self.basic_metrics.total_time:<20.2f}s {self.meve_metrics.total_time:<20.2f}s")
        
        # Retrieval
        logger.info(f"{'Initial Retrieved':<30} {self.basic_metrics.chunks_retrieved:<20} {self.meve_metrics.chunks_retrieved:<20}")
        logger.info(f"{'Final Chunks':<30} {self.basic_metrics.chunks_final:<20} {self.meve_metrics.chunks_final:<20}")
        
        # Quality
        logger.info(f"{'Avg Relevance Score':<30} {self.basic_metrics.avg_relevance_score:<20.3f} {self.meve_metrics.avg_relevance_score:<20.3f}")
        
        # Size
        logger.info(f"{'Context Characters':<30} {self.basic_metrics.context_chars:<20,} {self.meve_metrics.context_chars:<20,}")
        logger.info(f"{'Context Tokens':<30} {self.basic_metrics.context_tokens:<20} {self.meve_metrics.context_tokens:<20}")
        
        if self.meve_metrics.token_budget:
            logger.info(f"{'Budget Efficiency':<30} {'N/A':<20} {self.meve_metrics.budget_efficiency:<20.1f}%")
        ...
        if self.meve_metrics.avg_relevance_score > self.basic_metrics.avg_relevance_score:
            logger.info(f"   ✅ MeVe has HIGHER average relevance (+{(self.meve_metrics.avg_relevance_score - self.basic_metrics.avg_relevance_score):.3f})")
        else:
            logger.info(f"   ⚠️  Basic RAG has higher avg relevance")
        
        if self.meve_metrics.context_chars < self.basic_metrics.context_chars:
            reduction = ((self.basic_metrics.context_chars - self.meve_metrics.context_chars) / self.basic_metrics.context_chars) * 100
            logger.info(f"   ✅ MeVe uses {reduction:.1f}% LESS context (more efficient)")
        else:
            logger.info(f"   ⚠️  MeVe uses more context")
        
        time_overhead = ((self.meve_metrics.total_time - self.basic_metrics.total_time) / self.basic_metrics.total_time) * 100
