...
from meve.phases.phase3_fallback import build_corpus_stats, execute_phase_3
...
class MeVeEngine:
    ...
    def run(self, query_text: str) -> tuple[str, list]:
        """
        ...
        Returns:
            Tuple of (final_context_string, final_chunks_list) for LLM consumption
        """
        # Validate data sources
        if not self.vector_store and not self.vector_db_client:
            logger.error("No vector data source configured")
            return (
                "Error: No vector data source configured. Please provide vector_store or vector_db_client.",
                [],
            )
